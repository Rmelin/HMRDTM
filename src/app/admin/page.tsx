...
export default async function AdminPage() {
  ...
  const visibleEvents = isSystemAdmin(user) ? allEvents : ownedEvents.map((row) => row.event);
  const { active: list, archived: archivedList } =
    partitionEventsByArchive(visibleEvents);
  ...
  return (
    <div className="app-view">
      ...
      {archivedList.length > 0 ? (
        <CollapsibleSection title={`Arkiv (${archivedList.length})`}>
          <div className="meal-grid">
            {archivedList.map((event) => (
              <article className="meal-card" key={event.id}>
                <div>
                  <span className="badge">Arkiveret</span>
                  <h2 style={{ marginTop: 10 }}>{event.title}</h2>
                  <p>{event.location || "Intet sted angivet"}</p>
                </div>
